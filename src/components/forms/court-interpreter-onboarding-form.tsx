...
import { createCourtInterpreterProfileAction } from "@/app/onboarding/actions";

export function CourtInterpreterOnboardingForm() {
  const [state, formAction, pending] = useActionState(
    createCourtInterpreterProfileAction,
    undefined,
  );
  ...
  return (
    <form action={formAction} className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <label className="flex flex-col gap-1">
          <span className="text-sm font-medium text-[var(--color-ink-muted)]">Bydliště — město</span>
          <input
            name="city"
            maxLength={80}
            className="rounded-md border border-[var(--color-line-strong)] px-3 py-2 text-sm focus:border-[var(--color-brand-700)] focus:outline-none"
      ...
      <fieldset className="flex flex-col gap-3">
        ...
        <p className="text-xs text-[var(--color-ink-soft)]">
          Sazby jsou orientační — autoškola si je u tebe potvrdí. Můžeš nechat prázdné a doplnit později.
        </p>
      </fieldset>
